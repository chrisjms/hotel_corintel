...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_message') {
    if (!featureEnabled('messaging')) {
        if ($isAjaxRequest) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'error' => 'La messagerie est désactivée pour cet établissement.']);
            exit;
        }
...
require_once HOTEL_ROOT . '/shared/includes/content-helper.php';
...
<body data-i18n-title="contact.pageTitle" data-header-style="<?= h($headerStyle ?? 'classic') ?>">
  <?php $currentPageSlug = 'contact'; include __DIR__ . '/includes/header.php'; ?>
